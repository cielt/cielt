<?php
/**
* The template for displaying all pages.
*
* This is the template that displays all pages by default.
* Please note that this is the WordPress construct of pages
* and that other 'pages' on your WordPress site will use a
* different template.
*
* Please see /external/starkers-utilities.php for info on get_template_parts()
*
* @package 	WordPress
* @subpackage 	Cielt
* @since 		Cielt 0.1
*/
?>
<?php get_template_parts( array( 'parts/shared/html-header', 'parts/shared/header' ) ); ?>
<div id="wrapper-main">
<!-- marquee thing -->
<?php get_template_part( 'parts/shared/marquee', 'page' ); ?>	

<!-- features -->
<div class="band-callouts">
	<div id="wrapper-callouts" class="max960-centered clearfix">
		<div class="band-ticker">
			<ul class="checkthis-too">
				<li><a href=""><span class="ico heart"></span>side things</a></li>
				<li><a href=""><span class="ico danger"></span>rejects</a></li>
			</ul>              
		</div>	
	</div>

</div>
<!-- MAIN CONTENT -->	
<div id="wrapper-page-content" class="max960-centered">
	<div class="box-inner clearfix">
		<!-- FEED -->
		<h3 class="upper"><?php the_title(); ?></h3>
		<p>My portfolio.  To view more stuff i&rsquo;ve made, i invite you also to visit <a href="">fun</a> and <a href="">rejects</a>.</p>
		<div class="wrapper-feedlist">
			<?php
				$paged = (get_query_var('paged')) ? get_query_var('paged') : 1;
				$args= array(
				'post_type' => 'folio_project',
				//'category_name' => '', // Change these category SLUGS to suit your use.
				'paged' => $paged
				);
				query_posts($args);
				if( have_posts() ) : ?>
			<!-- projects headline list / sidebar -->
			<div class="column leftcol skinny">
				<ol class="feedlist title-roll">
					<?php while ( have_posts() ) : the_post(); ?>
					<?php get_template_part( 'listing', get_post_type()."-headline" ); ?>
					<?php endwhile; ?>
				</ol>
			</div>
			
			<!-- thumbnail list -->
			<div class="column rightcol x-wide">
				<ol class="feedlist thumbnail-list clearfix">
					<?php while ( have_posts() ) : the_post(); ?>
					<?php get_template_part( 'listing', get_post_type() ); ?>
					<?php endwhile; ?>
				</ol>
			<?php endif; ?></div>
		</div>
	</div>
</div><!-- /#wrapper-page-content -->	
</div><!-- /#wrapper-main -->
<?php get_template_parts( array( 'parts/shared/footer','parts/shared/html-footer' ) ); ?>